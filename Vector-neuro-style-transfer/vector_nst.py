...
from xing_loss import XingLoss
from joint_loss import JointLoss

class VectorNST:

    def __init__(self, num_iter=100, output_dir="results", saving_verbose_lvl=1):
        pydiffvg.set_use_gpu(torch.cuda.is_available())
    # pydiffvg.set_use_gpu(False)
        self.device = pydiffvg.get_device()
        self.default_logger = lambda s="": print(s, flush=True)
        self.num_iter = num_iter
        self.output_dir = output_dir
        self.saving_verbose_lvl = saving_verbose_lvl

    def image_loader(self, image_path, canvas_width, canvas_height):
    ...
    def alpha_blending(self, img, alpha_blending_coef=1.0, method=1):
        if img.shape[-1] == 3:
            return img
        if method == 0:
            img = img[:, :, :3] * img[:, :, 3:] + \
                  alpha_blending_coef * torch.ones([img.shape[0], img.shape[1], 3]).to(self.device) * \
                  (1 - img[:, :, 3:])
            return img
        if method == 1:
            return img[:, :, :3]
    ...
    def get_contours(self, canvas_width, canvas_height, shapes, shape_groups):
        shape_groups_new = copy.deepcopy(shape_groups)
        with torch.no_grad():
            self.make_contoured_shape_groups(shape_groups_new)
            img = svg_render(canvas_width, canvas_height, shapes, shape_groups_new)
        return self.alpha_blending(img).permute(2, 0, 1).unsqueeze(0)


    def change_stroke_width(self, shapes, canvas_width, canvas_height, width_value=0.5):
        for shape in shapes:
            shape.stroke_width = torch.tensor(width_value)


    def make_contoured_shape_groups(self, shape_groups, fill_color=0.):
    ...
    def run_style_transfer(self, cur_logger,
        ...
        cur_logger(f'device={self.device}')
        ...
        cur_logger(f'joint_weight={joint_weight}')
        ...
        content_style_loss_vgg_model, style_losses, content_losses = \
            get_style_model_and_losses(style_img, content_img, self.device)
        content_style_loss_vgg_model.requires_grad_(False)

        target_contour = self.get_contours(canvas_width, canvas_height, content_shapes, content_shape_groups)
        ...
        perception_loss = LPIPS(device=self.device).to(self.device)
        ...
        while run[0] <= num_steps:
            ...
            contour = self.get_contours(canvas_width, canvas_height, shapes, shape_groups).detach()
            ...
            img = self.alpha_blending(img, alpha_coef, alpha_method).permute(2, 0, 1).unsqueeze(0).to(self.device)
                ...
                if isinstance(x, pydiffvg.Path):
                  ...
                  for control_point in x.num_control_points:
                    if control_point == torch.tensor(2):
                      curves_points.append(x.points[i:i+4, :])
                      if i+4 > x.points.size()[0]:
            ...
            weighted_joint = joint_score * joint_weight

            loss = weighted_perc_score + weighted_contour_score + \
                   weighted_style_score + weighted_content_score + weighted_xing + weighted_joint
            loss.backward()
            ...
            if run[0] % 1 == 0:
                ...
                cur_logger('Joint Loss: {:4f}'.format(joint_score.item()))
                cur_logger('Weighted Joint Loss: {:4f}'.format(weighted_joint.item()))
    ...
    def run_vector_nst(self, content_img_path, style_img_path):
        num_iter = self.num_iter
        saving_verbose_lvl = self.saving_verbose_lvl
        ...
        style_img = self.image_loader(style_img_path, canvas_width, canvas_height)
        style_img = self.alpha_blending(style_img.permute(1, 2, 0)).permute(2, 0, 1).unsqueeze(0).to(self.device)
        content_img = self.alpha_blending(content_img).permute(2, 0, 1).unsqueeze(0).to(self.device)
        ...
        alpha_coef = 1.0
        alpha_method = 1
        shapes, shape_groups = self.run_style_transfer(
    ...
    def run_vector_nst_multi_params(self, content_svg, style, style_mime_type,
        ...
        print("run vector-nst multiparams!")
        xing_weight = xing_loss_weight
        ...
        if style_mime_type == "image/svg+xml":
            ...
            style_tensor = self.image_loader(BytesIO(image_data), canvas_width, canvas_height)
        style_tensor = style_tensor.to(self.device)

        content_img = svg_render(canvas_width, canvas_height, shapes, shape_groups)
        style_img = self.alpha_blending(style_tensor.permute(1, 2, 0), alpha_coef, alpha_method)
        style_img = style_img.permute(2, 0, 1).unsqueeze(0).to(self.device)
        content_img = self.alpha_blending(content_img, alpha_coef, alpha_method)
        content_img = content_img.permute(2, 0, 1).unsqueeze(0).to(self.device)
        ...
        tic = time.perf_counter()
        shapes, shape_groups = \
            self.run_style_transfer(self.default_logger, "", name, content_img,
        ...
        toc = time.perf_counter()
        self.default_logger("+" * 50)
        self.default_logger(f"{name} is finished!")
        self.default_logger(f"Total Time: {toc - tic:0.4f} seconds")
        self.default_logger("+" * 50)
